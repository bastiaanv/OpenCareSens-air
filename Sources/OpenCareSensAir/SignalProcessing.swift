...
/// - Returns: SgResult containing sig_out[10], seq_out[10], frep_out[6]
public enum SignalProcessing {
    public static func smoothSg(sigIn: [Double], seqIn: [Int], frepIn: [Int],
                                newSig: Double, newSeq: Int, newFrep: Int,
                                wSgX100: [Int]) -> SgResult {
        // Compute total weight
        var totalWeight: Double = 0.0
        var weights: [Double] = Array(repeating: 0.0, count: 7)
        for i in 0..<7 {
        ...
        var sigBuf = Array(repeating: 0.0, count: 10)
        var seqBuf = Array(repeating: 0, count: 10)
        var frepBuf = Array(repeating: 0, count: 6)
        ...
        let ref = sigBuf[9]
        var sigOut = Array(repeating: 0.0, count: 10)
    ...
    public static func regressCal(input: [Double], output: [Double],
                                  slopeArr: [Double], yceptArr: [Double],
                                  n: Int, newInput: Double, newOutput: Double)
                                -> RegressionResult {
        var allIn = Array(repeating: 0.0, count: 8)
        var allOut = Array(repeating: 0.0, count: 8)
        var total: Int = 0
        ...
        if total >= 2 {
            let reg = MathUtils.fitSimpleRegression(input: allIn, output: allOut, count: total)
            newSlope = reg[0]
            newYcept = reg[1]
        } else {
            newSlope = 1.0
        ...
        // Copy results back
        var resultInput = Array(repeating: 0.0, count: 7)
        var resultOutput = Array(repeating: 0.0, count: 7)
        var resultSlope = Array(repeating: 0.0, count: 7)
        var resultYcept = Array(repeating: 0.0, count: 7)
    ...
    ///
    /// - Parameters:
    ///   - slope: slope value to check
    ///   - ycept: intercept value to check
    ///   - slopeMin: minimum slope bound
    ///   - slopeMax: maximum slope bound
    ///   - yceptMin: minimum intercept bound
    ///   - yceptMax: maximum intercept bound
    ///   - cornerOffset: offset for diagonal constraint
    /// - Returns: true if inside boundary
    public static func checkBoundary(slope: Double, ycept: Double,
                                     slopeMin: Double, slopeMax: Double,
                                     yceptMin: Double, yceptMax: Double,
                                     cornerOffset: Double) -> Bool {
    ...
    public static func smooth1qErr16(in: [Double], n: Int) -> [Double] {
        ...
        for k in 0..<n {
            ...
            for j in 0..<n {
                let angle = 2.0 * .pi * Double(k) * Double(j) / Double(n)
                cosSum += in[j] * MathUtils.cos(angle)
                sinSum += in[j] * MathUtils.sin(angle)
            }
            
            // Hann penalty weight
            let w = 2.0 - 2.0 * MathUtils.cos(2.0 * .pi * Double(k) / Double(n))
            ...
            for j in 0..<n {
                let angle = 2.0 * .pi * Double(k) * Double(j) / Double(n)
                out[j] += cosSum * MathUtils.cos(angle) + sinSum * MathUtils.sin(angle)
            }
        }
    ...
    ///
    /// - Parameters:
    ///   - nVal: number of valid values
    ///   - meanVal: mean value
    ///   - maxVal: maximum absolute value
    ///   - flagVal: current flag value
    ///   - seq: sequence number
    ///   - mode: mode flag
    ///   - value: current value
    ///   - absValue: absolute value
    ///   - runningMean: accumulated running mean
    ///   - runningMax: accumulated running max
    ///   - thresholdSeq: threshold sequence number
    ///   - multi1: multiplier 1
    ///   - multi2: multiplier 2
    /// - Returns: ThresholdResult with updated n, mean, max, and flag values
    public static func calThreshold(nVal: Int, meanVal: Double, maxVal: Double,
                                    flagVal: Int, seq: Int64, mode: Int,
                                    value: Double, absValue: Double,
                                    runningMean: Double, runningMax: Double,
                                    thresholdSeq: Int, multi1: Int, multi2: Int)
                                   -> ThresholdResult {
        var newN = nVal
        var newFlag = flagVal
    ...
    public static func err1TdTrioUpdate(dstTrio: inout [Double], dstTime: inout [Int64],
                                        dstFlag: inout [Int], srcTrio: [Double],
                                        srcTime: [Int64], srcFlag: [Int],
                                        breakFlags: [Int]) {
        for i in 0..<90 {
            for j in 0..<3 {
    ...
    public static func err1TdVarUpdate(dstSeq: inout [Int], dstVal: inout [Double],
                                       dstTime: inout [Int64], counts: inout [Int],
                                       srcVal: [Double], srcTime: [Int64]) {
        for i in 0..<90 {
            dstVal[i] = srcVal[i]
            dstTime[i] = srcTime[i]
            dstSeq[i] = 0
    ...
    public static func getKernelWeight(e: Int, d: Int) -> Double {
        if e < 45 {
    ...
    ///
    /// - Parameters:
    ///   - data90: input data (90 elements)
    /// - Returns: fitted values (90 elements)
    public static func irlsLoess(data90: [Double]) -> [Double] {
        var fitted90 = Array(repeating: 0.0, count: 90)
        var bisquareW = Array(repeating: 0.0, count: 90)
        var absResid = Array(repeating: 0.0, count: 90)
        ...
        for iter in 0..<3 {
            for e in 0..<90 {
                ...
                for d in 0..<90 {
                    let kw = getKernelWeight(e: e, d: d)
            ...
            for i in 0..<90 {
                absResid[i] = MathUtils.abs(data90[i] - fitted90[i])
            }
            
            let medianAr = MathUtils.quickMedian(absResid, count: 90)
            let threshold = medianAr * 6.0
            if threshold < 1e-30 { break }
            
            var hasNan = false
            for i in 0..<90 {
                let u = absResid[i] / threshold
                if u > 1.0 { u = 1.0 }
                let w = (1.0 - u * u)
                w = w * w
                bisquareW[i] = w
    ...
    public static func runningMedians(in30: [Double]) -> [Double] {
        var out30 = Array(repeating: 0.0, count: 30)
        
        for g in 0..<5 {
            let base = g * 6
            
            // Window of 3: grp[0..2]
            var tmp3 = Array(repeating: 0.0, count: 3)
            for i in 0..<3 { tmp3[i] = in30[base + i] }
            out30[base + 0] = MathUtils.mathMedian(tmp3, count: 3)
            
            // Window of 4: grp[0..3]
            var tmp4 = Array(repeating: 0.0, count: 4)
            for i in 0..<4 { tmp4[i] = in30[base + i] }
            out30[base + 1] = MathUtils.mathMedian(tmp4, count: 4)
            
            // Window of 5: grp[0..4]
            var tmp5 = Array(repeating: 0.0, count: 5)
            for i in 0..<5 { tmp5[i] = in30[base + i] }
            out30[base + 2] = MathUtils.mathMedian(tmp5, count: 5)
            
            // Window of 6: grp[0..5]
            var tmp6 = Array(repeating: 0.0, count: 6)
            for i in 0..<6 { tmp6[i] = in30[base + i] }
            out30[base + 3] = MathUtils.mathMedian(tmp6, count: 6)
            
            // Window of 5: grp[1..5]
            for i in 0..<5 { tmp5[i] = in30[base + 1 + i] }
            out30[base + 4] = MathUtils.mathMedian(tmp5, count: 5)
            
            // Window of 4: grp[2..5]
            for i in 0..<4 { tmp4[i] = in30[base + 2 + i] }
            out30[base + 5] = MathUtils.mathMedian(tmp4, count: 4)
        }
        return out30
    }
    ...
    public static func firFilterMedians(prev3: [Double], medians30: [Double]) -> [Double] {
        let firC = [-0.25, 1.0, 1.75, 2.0, 1.75, 1.0, -0.25]
        
        // Extended buffer: prev3[3] + medians30[30]
        var extended = Array(repeating: 0.0, count: 33)
    ...
    /// - Returns: 30 filtered values
    public static func perSampleHampelFilter(tranInA: [Double],
                                              prev5Raw: inout [Double],
                                              prev5Corrected: inout [Double],
                                              outlierFifo: inout [Int8]) -> [Double] {
        // Determine detection buffer
        var fifoSum: Int = 0
        for i in 0..<6 {
            fifoSum += Int(Math.abs(Double(outlierFifo[i])))
        }
        let prev5 = (fifoSum >= 4) ? prev5Corrected : prev5Raw
        
        // Build detection buffer: [prev5, tranInA] = 35 values
        var buffer = Array(repeating: 0.0, count: 35)
        for i in 0..<5 { buffer[i] = prev5[i] }
        for i in 0..<30 { buffer[5 + i] = tranInA[i] }
        
        // Build replacement buffer
        var replBuf = Array(repeating: 0.0, count: 35)
        for i in 0..<5 { replBuf[i] = prev5Corrected[i] }
        for i in 0..<30 { replBuf[5 + i] = tranInA[i] }
        
        var perSample = Array(repeating: 0.0, count: 30)
        for i in 0..<30 { perSample[i] = tranInA[i] }
        
        for i in 0..<30 {
            // Sliding window of 6 consecutive values from buffer[i:i+6]
            var window = Array(repeating: 0.0, count: 6)
            ...
            // Compute MAD
            var absDev = Array(repeating: 0.0, count: 6)
            for j in 0..<6 { absDev[j] = MathUtils.abs(window[j] - median) }
            for a in 0..<5 {
                for b in (a + 1)..<6 {
            ...
            if mad >= 1e-14 {
                ...
                var meanAd: Double = 0
                for j in 0..<6 { meanAd += MathUtils.abs(window[j] - median) }
                meanAd /= 6.0
                if meanAd > 0.001 {
    ...
    public static func computeTranInA1min(tranInA: [Double],
                                          history60: inout [Double],
                                          prev3: inout [Double],
                                          prev5Raw: inout [Double],
                                          prev5Corrected: inout [Double],
                                          outlierFifo: inout [Int8],
                                          callCount: Int64,
                                          timeGap: Double) -> [Double] {
        // Step 1: Per-sample outlier removal
        var perSample: [Double]
        if callCount >= 2 {
            perSample = perSampleHampelFilter(tranInA: tranInA,
                                              prev5Raw: &prev5Raw,
                                              prev5Corrected: &prev5Corrected,
                                              outlierFifo: &outlierFifo)
        } else {
            perSample = Array(repeating: 0.0, count: 30)
            for i in 0..<30 { perSample[i] = tranInA[i] }
        ...
        if callCount >= 3 && timeGap < 897.2 {
            var data90 = Array(repeating: 0.0, count: 90)
            for i in 0..<60 { data90[i] = history60[i] }
            for i in 0..<30 { data90[60 + i] = perSample[i] }
            
            let fitted90 = irlsLoess(data90: data90)
            intermediate30 = Array(repeating: 0.0, count: 30)
        ...
        let medians30 = runningMedians(in30: intermediate30)
        
        // Step 4: FIR filter
        var firOut: [Double]
        if callCount >= 2 && timeGap < 327.2 {
            firOut = firFilterMedians(prev3: prev3, medians30: medians30)
        } else {
            firOut = Array(repeating: 0.0, count: 30)
            for i in 0..<30 { firOut[i] = medians30[i] }
        }
        
        // Step 5: calAverageWithoutMinMax per group of 6
        var tranInA1min = Array(repeating: 0.0, count: 5)
        for g in 0..<5 {
            var group = Array(repeating: 0.0, count: 6)
            for i in 0..<6 { group[i] = firOut[g * 6 + i] }
            tranInA1min[g] = MathUtils.calAverageWithoutMinMax(group, count: 6)
        }
        
        // Step 6: Update state
        // Shift history: [0:30] = [30:60], [30:60] = perSample
        for i in 0..<30 { history60[i] = history60[30 + i] }
